...
class AcGameObject {
    constructor(hurtable = false) // 构造函数
    {
        ...
        this.uuid = this.create_uuid(); // 对每个运动的物体设置一个唯一的uuid
    }
...
class ChatField {
    ...
    show_input() {
        ...
        this.$input.show();
        this.$input.focus();  // 输入聚焦于input文本框
    }

    hide_input() {
        this.$input.hide();
        this.playground.game_map.$canvas.focus();  // 退出重新聚焦game_map
    }
}
...
class Player extends AcGameObject {
    constructor(playground, x, y, radius, color, speed, character, username, photo) {
        ...
        if (this.character === "me") {  // 是自己则渲染技能图片
    ...
    start() {
        ...
        if (this.playground.player_count >= 2) {   // 设置每局人数
    ...
    destroy_fireball(uuid) {  // 销毁火球，在被攻击者内调用攻击者的销毁函数
    ...
    update_move() {  // 更新玩家移动
        this.spent_time += this.timedelta / 1000;
        // 增加前四秒的保护期
        // 这里update是每一帧执行一次，一般是每秒60帧，期望5秒发射一次，也就是1/300的概率
        ...
        if (this.damage_speed > this.eps) {  // 受到伤害则会失控一段距离
            this.vx = this.vy = 0;
            this.move_length = 0;
            ...
            if (this.move_length < this.eps) {  // 剩余移动距离为0
                this.move_length = 0;
                this.vx = this.vy = 0;
                if (this.character === "robot") {  // robot则重新给随机位移
                    let tx = Math.random() * this.playground.width / this.playground.scale;
                    let ty = Math.random() * this.playground.height / this.playground.scale;
                    this.move_to(tx, ty);
                }
...
class MultiPlayerSocket {
    constructor(playground) {
        ...
        this.ws = new WebSocket("wss://app701.acapp.acwing.com.cn/wss/multiplayer/");
    ...
    receive() {
        ...
        this.ws.onmessage = function (e) {
            let data = JSON.parse(e.data);  // 前端转换的函数parse
            let uuid = data.uuid;
    ...
    send_move_to(tx, ty) {  // 向服务器传递当前用户运动，在player里面调用
        let outer = this;
        this.ws.send(JSON.stringify({
            'event': "move_to",
            'uuid': outer.uuid,
    ...
    receive_move_to(uuid, tx, ty) {  // 接受并更新服务器其他用户的移动

        let player = this.get_player(uuid);  // 确定需要移动的player
